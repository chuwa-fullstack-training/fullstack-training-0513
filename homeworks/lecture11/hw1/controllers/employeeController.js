...
const getAllEmployee = async (req, res) => {
    try {
      ...
      const isAuthenticated = req.employee;

      const fieldsToRetrieve = isAuthenticated
      ? {}
      : { firstName: 1, lastName: 1 };

      const employees = await Employee.find(fieldsToRetrieve);
...
const getCompanyEmployees = async (req, res) => {
  try{
    ...
    if (!employee) {
      ...
      const company = await Company.findById(employee.company._id).populate('employees');
      if (!company) {
...
module.exports = { getAllEmployee, getOneEmployee, createEmployee, updateEmployee, deleteEmployee, getCompanyEmployees };