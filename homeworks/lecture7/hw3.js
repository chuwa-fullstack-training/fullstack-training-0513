    ...
    if (url === '/') {
      ...
      fs.readFile(path.join(__dirname, 'home.html'), 'utf8', (err, html) => {
        if (err) {
          ...
          const urlInfo = _url.parse(req.url);
          const params = querystring.parse(urlInfo.query);
          const content = (params.title && params.content) ? `<p>title: ${params.title || ''}, content: ${params.content || ''}</p>` : '';
          const _html = html.replace('</body>', `${content}</body>`);
